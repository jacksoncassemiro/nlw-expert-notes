...
export function NewNoteCard({ onNoteCreated }: NewNoteCardProps){
  ...
  const [shouldShowOnborarding, setShouldShowOnborarding] = useState(true)
  ...
  function handleStartRecording(){
    ...
    if(!isSpeechRecognitionAPIAvailable){
      alert('Infelizmente seu navagador nao suporta a API de gravação!')
      return
    }
    ...
    <Dialog.Root>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className='fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] md:h-[60vh] w-full bg-slate-700 md:rounded-md flex flex-col outline-none'>
          ...
          <Dialog.DialogClose className='absolute right-0 top-0 bg-slate-800 p-1.5 text-slate-400 hover:text-slate-100'><X className='size-5' /></Dialog.DialogClose>
